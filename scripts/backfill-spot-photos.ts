// scripts/backfill-spot-photos.ts
...
// Your Firebase config (copy from your firebase.ts)
...
async function fetchAndUploadGooglePlacePhoto(
  placeId: string,
  spotId: string
): Promise<string | null> {
...
async function backfillSpotPhotos() {
  ...
  try {
    // Query 1: Spots with no photoURLs field
    const spotsWithoutPhotosQuery1 = query(
      collection(db, 'spots'),
      where('status', '==', 'approved')
    );

    const snapshot = await getDocs(spotsWithoutPhotosQuery1);
